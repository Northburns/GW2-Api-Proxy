...
import fi.northburns.northburns.gw2.client.myclient.MyCache
...
class EndpointEntity<K, V>(
    ...
    fun getIds(): Set<K> {
        return if (cache.isAllKeysKnown()) cache.getKnownKeys()
            ...
            val ids = client.execute(allIds).decodingResult.getOrThrow().toSet()
